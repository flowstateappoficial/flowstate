// Flowstate — Send Referral Invite
// Envia convite por email para um amigo, via Resend.
// Deploy: supabase functions deploy send-referral-invite
//
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ── Buscar/gerar código de referral do utilizador ──
    ...
    // Nome do referrer (para personalizar o email).
    const referrerName =
    ...
    // ── Regista pedido (linha pendente na tabela referrals) ──
    // Ajuda a mostrar "invitesSent" imediato mesmo antes do signup do amigo.
    ...
    await supa.from('referrals').insert({
      ...
      // Ignora erros de unique (se já existir convite para este email).
